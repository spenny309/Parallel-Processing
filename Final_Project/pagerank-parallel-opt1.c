/*
  ...
  locking and barriers. This version implements the first major optimization to try to
  increase the efficiency.

  The major improvement in this version is to pre-calculate the range of nodes that
  each thread is responsible for looking at instead of needing to re-calculate this
  range in each thread for each iteration.
*/
